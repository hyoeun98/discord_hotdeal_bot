...
def capture_and_send_screenshot(driver, file_name):
    ...
    try:
        ...
        """
        cursor.execute(
            insert_query, (file_name, page_source, timestamp, "scanning error")
        )
        conn.commit()
    ...
    def get_comment_count(self, item):
        ...
        try:
            comment_count = item.find_element(
                By.CLASS_NAME, s_config["comment_count_class_name"]
            )
    ...
    def get_item_links(self):
        ...
        for i in range(1, 31):
            try:
                ...
                item = self.get_item_driver.find_element(
                    By.CSS_SELECTOR, find_item_css_selector
                )
                item_link = item.find_element(
                    By.CSS_SELECTOR, find_item_link_css_selector
                ).get_attribute("href")
                ...
                capture_and_send_screenshot(
                    self.get_item_driver, self.__class__.__name__
                )
                break
...
class ARCA_LIVE(PAGES):
    ...
    def get_comment_count(self, item):
        ...
        try:
            comment_count = item.find_element(
                By.XPATH, s_config["find_comment_count_xpath_selector"]
            )
    ...
    def get_item_links(self):
        ...
        for i in range(
            2, 27
        ):  # TODO: Consider moving loop range to selectors.yml if it varies often
            try:
                item_link = "err"
                find_item_xpath_selector = s_config["find_item_xpath_selector"].format(
                    i=i
                )
                find_item_link_xpath_selector = s_config[
                    "find_item_link_xpath_selector"
                ]

                item = self.get_item_driver.find_element(
                    By.XPATH, find_item_xpath_selector
                )
                item_link = item.find_element(
                    By.XPATH, find_item_link_xpath_selector
                ).get_attribute("href")
                self.item_link_list.append(item_link)
                comment_count = self.get_comment_count(item)
    ...
    def get_item_links(self):
        ...
        for item in soup.find_all(class_=s_config["item_list_classes"])[:20]:
            try:
                ...
                item_link = (
                    s_config["item_link_href_prefix"] + item_link_element.attrs["href"]
                )
                item_link = item_link.replace(
                    "&&", "&"
                )  # This specific string replacement might be better handled if it's a common pattern
                self.item_link_list.append(item_link)
                comment_count = self.get_comment_count(item)
...
class EOMI_SAE(PAGES):
    ...
    def get_comment_count(self, item):
        ...
        try:
            comment_count = item.find_element(
                By.CSS_SELECTOR, s_config["comment_count_css_selector"]
            ).text
            print(
                comment_count
            )  # This print might be for debugging, consider removing for production
            comment_count = int(comment_count)
    ...
    def get_item_links(self):
        ...
        for i in range(2, 22):  # TODO: Consider moving loop range to selectors.yml
            try:
                ...
                item_link = item.find_element(
                    By.CLASS_NAME, find_item_link_class_name
                ).get_attribute("href")
                self.item_link_list.append(item_link)
                comment_count = self.get_comment_count(item)
